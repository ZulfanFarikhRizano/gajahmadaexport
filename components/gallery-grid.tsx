...
export function GalleryGrid({ products }: GalleryGridProps) {
  const [selectedGroup, setSelectedGroup] = useState<"all" | "indoor" | "outdoor">("all");

  // Filter presisi menggunakan properti 'group' dari CATEGORIES
  const filteredProducts = products.filter((product) => {
    if (selectedGroup === "all") return true;

    const matchedCategory = CATEGORIES.find((c) => c.slug === product.category);
    return matchedCategory?.group === selectedGroup;
  });

  return (
    <div className="mx-auto max-w-6xl px-6">
      {/* Filter Tabs (Semua, Indoor, Outdoor) */}
      <div className="flex flex-wrap items-center justify-center gap-2 mb-10">
        {[
          ...
          <button
            key={tab.id}
            onClick={() => setSelectedGroup(tab.id as "all" | "indoor" | "outdoor")}
      ...
      {filteredProducts.length === 0 ? (
        <p className="text-center text-clay-600 py-12">
